...
maxDistance        = 0.12;   % only roughly tuned at this point
...
vhat_MLESAC        = NaN*ones(NScans,3);
vhat_ODR_weighted  = NaN*ones(NScans,3);

% init covariance matrices
sigma_odr_weighted = NaN*ones(NScans,3);
    ...
    if Ntargets_valid > 2
        ...
        if Ntargets_valid > 1 %&& Nbins_valid > 1
            ...
            if (Ntargets_inlier > 2) % && (Nbins_inlier > 1)
                ...
                [ model_odr_weighted, ~, cov_weighted ] = ODR_3D( doppler(inlier_idx'), ...
...
% plot(ax_h(11),twist_time_second,twist_linear_body(:,3), ...
%     'color',colors(2,:),'LineWidth',1);
